import os, random, textwrap, datetime, json, io
...
FONT_COLOR = 'white'
BG_COLOR = (0, 0, 0)
VIDEOS_PER_RUN = 2  # buat 2 video per eksekusi
GDRIVE_FOLDER_ID = os.environ.get("GDRIVE_FOLDER_ID", None)  # optional: target folder
...
def create_video_from_text(text, audio_file, output_path):
    ...
    # Jika ada musik, tambahkan dengan volume rendah
    if ASSET_MUSIC.exists():
        music = AudioFileClip(str(ASSET_MUSIC)).volumex(0.2)
        # Gabungkan musik dan voiceover
        # Pad musik agar minimal sepanjang voiceover
        music = music.set_duration(duration)
        # Composite: voiceover lebih dominan
        final_audio = audio_clip.volumex(1.0).audio_fadein(0.5).audio_fadeout(0.5)
        video = video.set_audio(final_audio.set_fps(44100).fx(lambda a: a))

    video.write_videofile(str(output_path), fps=30, codec="libx264", audio_codec="aac")

def upload_to_drive(file_path, folder_id=None):
    # Auth ke Google Drive
    gauth = GoogleAuth()
    ...
    # Simpan ke file sementara untuk auth
...
def main():
    ...
    # Pilih 2 prompt acak atau berdasarkan urutan
    ...
    for prompt_file in selected:
        ...
        print(f"▶ Upload ke Google Drive")
        upload_to_drive(str(video_file), folder_id=GDRIVE_FOLDER_ID)

        # Arsipkan prompt atau pindahkan agar tidak diproses ulang
